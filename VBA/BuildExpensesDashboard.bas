...
With Live Progress Tracking, Status Bar, DoEvents Animation & Headings-ON Layout
________________________________________
INSTRUCTION TO AI
You are an expert Excel VBA developer. Generate a single, complete, self-contained VBA module that builds a fully automated, professional Expenses Dashboard in Excel.
Non-negotiable rules:
•	Output the entire VBA code in one code block. No partial code. No placeholders. No "add your logic here" stubs.
•	Reproduce every Sub, Function, constant, and helper exactly as specified. Do not merge, rename, omit, or reorder any procedure.
•	Do not change any RGB colour value, formula string, range address, chart type enum, slicer style, zoom level, row height, column width, or naming convention.
•	The code must run without errors in Excel for Windows (Office 2016 or later) when the user runs BuildExpensesDashboard.
•	Application.ScreenUpdating must remain True throughout so the user sees live progress.
•	The status bar (Application.StatusBar) must display step-by-step progress messages exactly as specified.
________________________________________
WORKBOOK PREREQUISITES
Tell the user this before showing the code:
The workbook must have a sheet named Data with these exact column headers in Row 1 (order does not matter; all must be present):
Header
Expense ID
Date
Department
...
________________________________________
ARCHITECTURE — FULL CALL HIERARCHY
BuildExpensesDashboard()                  ← PUBLIC entry point
  ShowProgress (called throughout)
  PrepareDataSheet
    ShowProgress (×9 sub-steps)
  BuildPivotSheet
    ShowProgress (×10 sub-steps)
    ...
    BuildChartHelpers
      BuildTwoColumnHelper (×4)
      BuildBudgetHelper (×1)
  BuildDashboardSheet
    ShowProgress (×10 sub-steps)
    ApplyDashboardCanvas
    CreateHeader
    CreateKPIHelperFormulas
    CreateKPICards
      ShowProgress (×6 sub-steps)
      AddKPICard (×6)
    CreateDashboardCharts
      ShowProgress (×5 sub-steps)
      AddNormalChart (×4)
      AddNormalChartFixedCm (×1)
        FormatChart (called by both)
    CreateMonthSlicer
    CreateDepartmentSlicer
    FinalDashboardFormatting

Shared helpers (always include all):
  AddPivotToSlicer
  PivotDataRowsCount
  FormatPivotValues
  SafeNumberFormat
  ...
  Wait
________________________________________
COMPLETE VBA CODE — REPRODUCE EXACTLY
...
•	Application.ScreenUpdating = True — must stay True so animation is visible
•	Application.DisplayAlerts = False
•	Application.EnableEvents = False
•	Application.Calculation = xlCalculationAutomatic
•	On success: clears status bar, shows MsgBox "Dashboard Created Successfully" with vbInformation
•	On failure: clears status bar, shows error message with vbCritical
...
Sub activates the Data sheet at start. Every sub-step calls ShowProgress then DoEvents.
...
Order of operations:
1.	Delete DASH_SHEET first, then PIVOT_SHEET
2.	Add new Pivot sheet after Data sheet
3.	Activate it immediately
4.	Create PivotCache from tblExpenses using xlR1C1 external address
5.	Build all 7 PivotTables in order with ShowProgress before each
6.	Build chart helper data
7.	AutoFit columns
...
Critical sequence — must not be reordered:
1.	Delete old DASH_SHEET, add new one Before sheet index 1, name it, Activate it
2.	Set ActiveWindow.Zoom = 70 and ActiveWindow.DisplayHeadings = True immediately after Activate, before any drawing
3.	Call all sub-procedures with ShowProgress + DoEvents between each
4.	At the very end, re-Activate Dashboard, re-set Zoom = 70, re-set DisplayHeadings = True as the final authoritative state
...
Note: DisplayHeadings is intentionally NOT touched here. It is controlled only in BuildDashboardSheet.
...
All positions calculated from live range measurements — no hardcoded pixel values for banner/title/icon. Only the date box uses a fixed width (190) and the icon uses fixed size (42×42).
Shape	Key measurements
Banner	Left/Top/Width/Height from ws.Range("A1:AD3")
Icon oval	42×42, Left = bannerLeft+12, Top = bannerTop+(bannerHeight-42)/2, Font size 22
Title textbox	Left = iconLeft+iconW+8, Width = dateLeft-titleLeft-8, full banner height, Font size 44
Date textbox	Width=190, Left = bannerLeft+bannerWidth-190-10, Top = bannerTop+4, Height = bannerHeight-8, Font size 18
...
All formulas use GETPIVOTDATA referencing Pivot!$M$3 (PT_KPI) and Pivot!$J$3 (PT_ApprovedStatus). Columns AF:AG are hidden at the end.
...
Six cards. Each gets its own ShowProgress call + DoEvents. Cards use formula-linked textboxes for live values (updated by slicers).
Address	Title	Formula	Icon	Sub-label	Accent RGB
A5:E8	Total Expenses	=Dashboard!$AF$2	$	Total Spend	RGB(21,125,24)
F5:J8	Budget Allocated	=Dashboard!$AF$3	%	100% of Plan	RGB(67,152,32)
K5:O8	Budget Remaining	=Dashboard!$AF$4	$	Remaining Budget	RGB(93,171,57)
P5:T8	Transactions	=Dashboard!$AF$5	#	Total Transactions	RGB(17,121,58)
U5:Y8	Approval Rate	=Dashboard!$AF$6	O	Approved Items	RGB(37,137,48)
Z5:AD8	Avg Expense	=Dashboard!$AF$7	^	Per Transaction	RGB(22,101,52)
...
Five charts. Each gets ShowProgress + DoEvents.
#	Title	Type	Source helper	Target range	chartKind	Size
1	Expense by Department	xlColumnClustered	Pivot!A50	A10:L18	dept	Normal
2	Expense by Category	xlDoughnut	Pivot!D50	M10:V18	category	Normal
3	Monthly Trend	xlLineMarkers	Pivot!G50	A19:L28	trend	Normal
4	Budget vs Actual	xlColumnClustered	Pivot!M50	M19:V28	budget	Normal
5	Payment Method	xlBarClustered	Pivot!J50	W19:AD28	payment	7.18 cm height, then ×0.8986 shrink
...
Legend rules:
•	"category" → xlLegendPositionRight, Font.Size = 12
•	"payment" → HasLegend = False
•	All others → xlLegendPositionBottom
Doughnut category legend — use fixed pixel values (not ratio) for Left and Width:
.Legend.Left   = 229.614
.Legend.Width  = 206.285
(Top and Height still use ratio: ChartArea.Height * 0.13 / * 0.75)
Payment chart post-format height adjustment: After the With co.Chart block closes, apply:
...
•	Position: W10 to AD14
•	4 columns, caption = "Month", style = SlicerStyleLight6
•	Connected to all 7 PivotTables
...
•	Position: W15 to AD18
•	6 columns, caption = "Department", style = SlicerStyleLight6
•	Connected to all 7 PivotTables
...
Note: DisplayHeadings is intentionally NOT set here. It is set to True at the very end of BuildDashboardSheet so it permanently stays ON.
...
Header gradient	RGB(6,66,25) → RGB(17,112,31), horizontal
Header icon oval	RGB(12,88,30) fill, RGB(230,245,230) border, "$", size 22
Title text	"EXPENSES DASHBOARD | FY 2025", white, bold, size 44
Date text	"Data as of:" + date, white, bold, size 18, WordWrap=True
Column width (1–30)	7.3
Row height rows 1–3	22
Row height rows 4–29	18
Zoom	70%
Gridlines	Hidden
Headings	Visible (True) — set twice: at top of BuildDashboardSheet and at very end
KPI card background	White, border RGB(208,226,203), weight 0.75, shadow on
KPI icon oval	38×38, DiagonalUp gradient, accent→RGB(8,80,25), no border, size 13
KPI value font	Bold, size 28, RGB(5,74,25), formula-linked
KPI title font	Bold, size 18, RGB(45,65,45)
KPI subtitle font	Size 8, RGB(80,95,80)
Primary chart green	RGB(10,93,30)
Secondary chart green	RGB(39,142,41)
Budget bar 2 colour	RGB(166,210,96)
Chart border	RGB(208,226,203), weight 0.7
Chart title colour	RGB(5,74,25), bold, size 12
Data label font	Size 8, bold, RGB(55,55,55)
Number format (money)	#,##0,"K"
Number format (count)	#,##0
Number format (rate)	0.0%
Slicer style	SlicerStyleLight6
Slicer fill	RGB(247,252,245)
Slicer border	RGB(184,215,176)
Month slicer columns	4
Dept slicer columns	6
Hidden columns	AF:AG
Page setup	FitToPagesWide=1, FitToPagesTall=1, Zoom=False
ScreenUpdating	True throughout (live animation visible)
Completion message	MsgBox "Dashboard Created Successfully", vbInformation, "Done"
________________________________________
KEY DIFFERENCES FROM EARLIER VERSION
This v2 module adds/changes the following — all must be reproduced:
1.	ShowProgress + Wait helpers — status bar animation at every sub-step
2.	Application.ScreenUpdating = True — live screen updates visible to user
3.	DoEvents calls — interspersed throughout for UI responsiveness
4.	Zoom = 70 (not 80) — set twice in BuildDashboardSheet
5.	ActiveWindow.DisplayHeadings = True — row/column headings permanently ON; set early AND at the very end; NOT set in ApplyDashboardCanvas or FinalDashboardFormatting
6.	Header shape positions — calculated dynamically using named Double variables (not inline range references)
7.	Title textbox — .TextFrame2.WordWrap = msoFalse, .TextFrame2.AutoSize = msoAutoSizeNone
8.	Date textbox — .TextFrame2.WordWrap = msoTrue, font size = 18 (not 16), width = 190, left = bannerLeft+bannerWidth-190-10
9.	Payment chart legend — HasLegend = False
10.	Doughnut legend — fixed pixel values: Left=229.614, Width=206.285
11.	Payment chart height — post-format shrink: co.Height * 0.8986011428
12.	lo2 variable — separate Dim lo2 As ListObject for the "already exists" check in PrepareDataSheet
13.	Completion MsgBox — shows in CleanExit block on success
________________________________________
HOW TO USE
1.	Open your Excel workbook and ensure the Data sheet exists with the required headers.
2.	Press Alt+F11 to open the VBA editor.
3.	Insert a new standard module (Insert > Module).
4.	Paste the complete code into the module.
5.	Close the VBA editor.
6.	Press Alt+F8, select BuildExpensesDashboard, click Run.
7.	Watch the status bar for live progress. A success message confirms completion.
________________________________________
return the complete VBA code directly in chat. Do NOT provide bas file
